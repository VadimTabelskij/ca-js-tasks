...
console.groupCollapsed('8. Iš students masyvo atrinkti ir atspausdinti visų studentų vidurkius');
{
  ...
  students.forEach((student) => {
    ...
    student.modules.forEach((modules) => {
      rezultCredit += modules.credits;
      const sumMarks = modules.marks.reduce((accumulator, currentValue) =>
        accumulator + currentValue, 0);
      const avgMarks = sumMarks / modules.marks.length;
      rezultMarks += modules.credits * avgMarks;
    });

    let rezultAll = rezultMarks / rezultCredit;

    console.log(`${student.name} ${student.surname} ${rezultAll.toFixed(1)}`);
  });
...
console.groupCollapsed('9. Atspausdinti visų Informatikos fakulteto studentų vidurkius');
{
  ...
  const studentsInformation = students.filter((student) => student.faculty === 'Informatikos fakultetas');
  ...
  studentsInformation.forEach((student) => {
    ...
    console.log(`${student.faculty} - ${student.name} ${student.surname} = ${rezultAll.toFixed(1)}`);
...
{
  const studentsElectricityAndElectronics = students.filter((student) => student.faculty === 'Elektros ir elektronikos fakultetas');

  studentsElectricityAndElectronics.forEach((student) => {
    let rezultMarks = 0;
    let rezultCredit = 0;
